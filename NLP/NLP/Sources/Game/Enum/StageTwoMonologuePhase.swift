...
//    case giveOrTalkChoice
    ...
    var monologue: [(String, Color)] {
        switch self {
        case .stageArrived:
            return [("드디어 중앙통제실이다. 산소도, 조명 문제도 해결돼서 다행이야. 핀이랑 제인은 도대체 어디 있는거야...", NLPColor.label)]
        case .meetBot:
            return [("이건 제인의 장난감인데... 도움이 될 지는 모르겠지만 한 번 대화를 해보자.", NLPColor.label)]
        case .botUselessThought:
            return [("전혀 도움이 안 된다. 제인이 공감모드를 실험한다고 만든건데, 충돌때 망가진 것 같다. 어떻게 해야 하지?", NLPColor.label)]
        case .tryEmotionalApproach:
            return [("공감밖에 하지 못하는 로봇이니까 대화를 건네볼까? 아니면 로봇을 감동시킬 수 있는 방법이 있을까?", NLPColor.label)]
//        case .giveOrTalkChoice:
//            return "이건 아닌 것 같아. 줄 만한게 없는데...손전등이라도 줘볼까?"
        ...
        case .botBehaviorShiftNoticed:
            return [("뭔가 표정이 바뀐 것 같기도 하고... 다시 말을 걸어보자.", NLPColor.label)]
